...
extern QueueHandle_t Q_song_name;
...
app_cli_status_e cli__play(app_cli__argument_t argument, sl_string_t user_input_minus_command_name,
  ...
  song_name name;
  uint8_t i = 0;
  while ((user_input_minus_command_name[i] != '\0') && (i < 31)) {
    name[i] = user_input_minus_command_name[i];
    ++i;
  }
  if (i != 31) {
    name[i] = '\0';
    if (xQueueSend(Q_song_name, (void *)&name, 10)) {
      cli_output(NULL, "Queue send success\n");
      return APP_CLI_STATUS__SUCCESS;
    }
  }
  return APP_CLI_STATUS__SUCCESS;
